...
This is the portable advanced guide for deciding how data should enter a
MOON/COSMOS+ network-scoring workflow. Its maintained source is
[`cosmosR/agent-docs/moon-data-pkn-mapping-principles.md`](https://github.com/saezlab/cosmosR/blob/master/agent-docs/moon-data-pkn-mapping-principles.md);
prefer that source when its current version differs. The central question is:
...
The guide is based on the COSMOS+ preprint, especially sections 2.1, 2.3, and
4.4, and uses the Factor_COSMOS tutorial code, especially the
`Filtering_RNA_weights` section of:

https://github.com/saezlab/Factor_COSMOS/blob/main/MOFA_to_COSMOS.Rmd

See [the latent-factor guide](moon-latent-factor-principles.md) for the
separate question of how to select and orient latent factors before applying
these mapping rules.
...
> **Current `cosmosR` caveat.** The current `moon()` implementation does not
> force an `upstream_input` node into a result or impose its supplied sign; it
> only sign-filters an overlapping scored node. Treat the intervention rules
> below as causal-modeling principles, not as functionality automatically
> enforced by the current package. Do not report a lesion or knockdown as a
> hard MOON constraint without a separately implemented and validated
