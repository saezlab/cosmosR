...
The legacy CARNIVAL workflow now requires an explicit TF-target regulon with
`tf`, `sign`, and `target` columns. The small one-edge regulon below is only
for the packaged toy example. For a new analysis, prepare a CollecTRI regulon
before calling the preprocessing function. A cached `decoupleR` CollecTRI
table uses `source`, `target`, and `mor`, so convert it with
