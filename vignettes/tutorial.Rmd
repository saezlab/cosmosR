...
network_plot
```
This network represents the flow of activities that can connect FOXM1 up-regulation with glutathione (CID 124886)
accumulation. Here, FOXM1 can activate MYC, which in turn activate BCAT1. The activation of BCAT1 can lead 
to the increased production of glutamate (CID 33032), which in turn can be converted to glutathione GGT enzymes.
...
In this section we perform an Over Representation Analysis (ORA) by determining 
whether the genes (nodes) in the COSMOS solution network show statistically
significant differences in comparison to the prior-knowledge network (PKN). 

In this way the significant differences between two biological conditions
(e.g. cancer vs. normal tissue, or treatment vs. untreated cells) can be shown.
...
To perform the ORA, two different lists are required:
...
cosmos_result <- list(weightedSIF = full_sif, nodesAttributes = full_attributes)

##Load all canonical pathways with annotated genes as Gene Symbols from MSigDB 
#(https://www.gsea-msigdb.org/gsea/index.jsp)
pathways = gmt_to_dataframe("./data/c2.cp.v7.4.symbols.gmt") 

## Extract nodes from COSMOS solution network and background as two lists
#'(without metabolite nodes, for performing a metabolite enrichment analysis, see:
#'https://github.com/saezlab/ocean/blob/master/tutorial_metabolite_enrichment_analysis.R)
nodes_cosmos = extract_COSMOS_nodes(cosmos_result)

##As toy data set is too small, we need more background genes
nodes_bg <- c("PNP", "ACAA1", "ACO1", "ADA", "AGER", "CSNK2A2", "DBF4B",
              "PIK3CA", "SMAD4", "AHCY", "AKT3", "ESR1", "GATA2", "SMAD3",
              "SRPK2", "BTK", "CDK9", "MAPK8")
nodes_cosmos$bg <- append(nodes_cosmos$bg,nodes_bg)

# The gene set analysis (GSA) is based on a list of significant genes
# (Cosmos nodes) and a gene set collection (background). 
# We perform the GSA with Fisher's exact test (from piano package)
sig_pathways <- runGSAhyper(genes = nodes_cosmos$sucesses, 
                            universe = nodes_cosmos$bg,
                            gsc = loadGSC(pathways),
                            adjMethod = "fdr")
sig_pathways_df <- as.data.frame(sig_pathways$resTab)  %>% 
                   tibble::rownames_to_column(var = "pathway") 

## Plot most significant pathways based on p-value
sig_pathways_plot = plot_pathways(sig_pathways_df,
                                  cutoff = 0.08)  #p-value <= cutoff
sig_pathways_plot

top_pathways_barplot = top_pathways(sig_pathways_df)
top_pathways_barplot
```

## Tutorial section: Highlight interesting pathways in Cytoscape network

We can use the result from the ORA and map the most significant pathway to every
node (i.e. protein, enzyme, TF, kinase and metabolite) in the solution network
(cosmos_result$nodesAttributes).
The user has to define a set of interesting pathways to which a color will be 
mapped. The border of a node in the solution network will be colored according 
to the pathway the node belongs to.

```{r, warning=FALSE, message=FALSE}
#' - Look at result of ORA and select the most significant pathways
#' - Or choose pathways of interest in a specific disease, e.g. cancer
##These are the available pathways to choose from
View(pathways)

pathways_interesting <- c("BIOCARTA_MAPK_PATHWAY",
                          "KEGG_PANTOTHENATE_AND_COA_BIOSYNTHESIS",
                          "REACTOME_LEISHMANIA_INFECTION",
                          "WP_IL2_SIGNALING_PATHWAY")

##Choose "p-value" or "Adjusted p-value" as filter method
cosmos_result$nodesAttributes <- highlight_pathways(pathwaysMSigDB = pathways,
                                             sigPathwaysDf = sig_pathways_df,
                                             cosmosResults = cosmos_result,
                                             pathwaysInt = pathways_interesting,
                                             filterMethod = "p-value")
