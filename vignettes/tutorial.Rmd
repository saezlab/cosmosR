...
##As toy data set is too small, we need more background genes, these are randoms
...
sig_pathways_df$pathway <- row.names(sig_pathways_df)
sig_pathways_df <- sig_pathways_df[,c(7,1:6)] # put the pathway column in first place
...
```
(Here, the pathway highlighted are random since they were generated from a toy example and a random background)
```{r, warning=FALSE, message=FALSE}
sessionInfo()
```
